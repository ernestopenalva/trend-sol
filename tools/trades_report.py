...
def _print_detail(records: list[Dict[str, Any]], config: Dict[str, Any], *, include_h2: bool = False) -> None:
    ...
    for record in records:
        fields = [
            ...
            f"be_armed_at={record.get('be_armed_at') or 'n/a'}",
            f"time_to_be={_fmt_optional_duration(record.get('time_to_be_seconds'))}",
            f"no_progress={record.get('no_progress_enabled')}",
            f"no_progress_tolerance={_fmt_optional_duration(record.get('no_progress_tolerance_seconds'))}",
            f"no_progress_source={record.get('no_progress_tolerance_source') or 'n/a'}",
