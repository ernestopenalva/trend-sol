...
def main() -> None:
    ...
    _print_distribution(points, args.capital)
...
def _signature(row: dict[str, Any]) -> str:
    ...
    return " | ".join((f"version={value('strategy_version')}", f"HS={value('hard_stop_pct')}", f"NPE={value('no_progress_enabled')}", f"PL_floor={value('profit_lock_economic_floor')}"))
...
def _print_distribution(points: list[EquityPoint], capital: float) -> None:
    ...
    print("metric | p50 | p75 | p90 | p95 | max")
...
def _quantile_line(values: list[float]) -> str:
    return " | ".join(f"{_quantile(values, q):+.3f}%" for q in (0.50, 0.75, 0.90, 0.95)) + f" | {max(values, default=0):+.3f}%"
