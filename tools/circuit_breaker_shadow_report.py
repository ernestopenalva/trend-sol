...
def _reopen_diagnostics(triggers:list[dict[str,Any]], events:list[dict[str,Any]], real:list[dict[str,Any]], cb:list[dict[str,Any]]) -> None:
    ...
    for item in triggers:
        ...
        if start and end: total_pause += (end-start).total_seconds()/3600
        blocked=[x for x in events if x.get('event')=='ENTRY_BLOCKED_CIRCUIT_BREAKER' and start and _ts(x.get('ts')) and _ts(x.get('ts'))>=start and (not end or _ts(x.get('ts'))<end)]
        ...
        replacements=[x for x in cb if end and _ts(x.get('opened_at')) and _ts(x.get('opened_at'))>=end]
        first=replacements[:3]; cb_only=[x for x in replacements if _key(x) not in { _key(x) for x in real }]
        print(f"trigger={_fmt_ts(item.get('trigger_time'))} | release={_fmt_ts(release.get('release_time'))} | blocked={len(blocked)} | matched REAL_A blocked net=${sum(_net(x) for x in blocked_real):+.4f} | first CB after release={','.join(_fmt_ts(x.get('opened_at')) for x in first) or 'none'} | CB-only substitutes={len(cb_only)} (${sum(_net(x) for x in cb_only):+.4f})")
