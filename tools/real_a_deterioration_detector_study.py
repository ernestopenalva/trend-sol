...
def _report(name,kind,rows,state,t0,trades,cap):
 ...
 print(f'{name} | {kind} | delay median/mean={_num(delays)}/{_mean(delays)}m | flips/day={flips/max(len(rows)/288,1):.2f} | state={sum(state)/len(state):.1%} | net/trade deteriorating/normal={_mean(bad):+.3f}%/{_mean(good):+.3f}% | sizing $15 avoided/sacrificed/net/ratio=${loss:.3f}/${sacrificed:.3f}/${loss-sacrificed:.3f}/{loss/sacrificed if sacrificed else 0:.2f} | N={len(tagged)}')
